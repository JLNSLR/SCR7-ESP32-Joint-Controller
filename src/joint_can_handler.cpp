...
void can_handle_task(void* params) {
    ...
    while (true) {
        ...
        if (counter % 2 == 0) { // 500 Hz
            can_send_drive_state(drvSys_get_drive_state());
        }

        if (counter % 100 == 0) { // 10 Hz
...
void can_send_controller_state(drvSys_controllerCondition controller_state) {
    ...
    drvComm_ControllerState state_data = drvComm_pack_controllerState(controller_state, max_motor_torque);
...
void can_send_drive_state(drvSys_driveState drive_state) {
    ...
    drvComm_DriveStatePaket drive_state_data = drvComm_pack_drive_state(drive_state);

    union {
        drvComm_DriveStatePaket data;
...
void can_send_pid_data(drvSys_parameters params) {
    ...
    paket_0 = drvComm_pack_pid_packet(params.pos_pid_gains, true);
    ...
    union {
        ...
        uint8_t bytes[DRVCOMM_PID_GAIN_LENGTH];
    }data_packed;

    CAN.beginPacket(can_id.msg_id);
    CAN.write(data_packed.bytes, DRVCOMM_PID_GAIN_LENGTH);
    CAN.endPacket();

    paket_0 = drvComm_pack_pid_packet(params.vel_pid_gains, false);
    ...
    if (packet_size) {
        ...
        if (can_id.joint_id == JOINT_ID) {
            ...
            if (can_id.msg_type == drive_traj_target) {
                ...
                drvSys_driveTargets drive_target = drvComm_unpack_traj_command(input_paket.traj_cmd);


                Serial.print("Parsed Drive Target Traj command ");
                Serial.print(drive_target.pos_target);
                Serial.print(" ");
                Serial.print(drive_target.pos_target);
                Serial.print(" ");
                Serial.print(drive_target.pos_target);
                Serial.print(" ");
                Serial.println(drive_target.pos_target);


                handle_motion_command(drive_target);
            ...
            if (can_id.msg_type == drive_direct_torque_command) {
                ...
                float target_torque = drvComm_unpack_direct_torque_command(input_packet.direct_cmd);
            ...
            if (can_id.msg_type == drive_pid_data) {
                union {
                    drvComm_PID_Gains_Paket gains_packet;
                    uint8_t bytes[DRVCOMM_PID_GAIN_LENGTH];
                }input_packet;

                CAN.readBytes(input_packet.bytes, size_t(DRVCOMM_PID_GAIN_LENGTH));

                drvSys_PID_Gains gains = drvComm_unpack_pid_packet(input_packet.gains_packet);

                if (input_packet.gains_packet.type == true) {
                    // Position PID gains

                    drvSys_set_PID_gains(true, gains.K_p, gains.K_i, gains.K_d);

                }
                else if (input_packet.gains_packet.type == false) {
                    // Velocity PID gains
                    drvSys_set_PID_gains(false, gains.K_p, gains.K_i, gains.K_d);
                }
            ...
            if (can_id.msg_type == reset_command) {
