 ...
 */
int AS5048A::getRotationCentered(bool median)
{
  word data;
  int rotation;
  data = AS5048A::getRawRotation(median);
...
uint16_t AS5048A::getRotationPos(bool median)
{
  word data;
  int rotation;
  data = AS5048A::getRawRotation(median);
  ...
  last_sample_raw = rotation;
  return rotation;
}

int32_t AS5048A::getRotation(bool median) {
  ...
  last_sample = rotation - angle_offset;

  return last_sample;

}

void AS5048A::setOffsetAngle(int32_t raw_positive_offset_angle) {
  this->angle_offset = raw_positive_offset_angle;
}
...
float AS5048A::getRotationRad(bool median) {
  ...
  return float((2 * PI) / float(AS5048A_ANGLE) * AS5048A::getRotation(median));
}
